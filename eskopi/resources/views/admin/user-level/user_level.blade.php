        ...
        <section class="section">
            <div class="section-header">
                ...
                <div class="section-header-breadcrumb">
                    ...
                    <div class="breadcrumb-item">{{ __('User Level') }}</div>
                </div>
            </div>
                    ...
                    <div class="col">
                        <div class="card">
                            <div class="card-header d-flex justify-content-between">
                                <h4>{{ __('User Level') }}</h4>
                                <div>
                                    <a href="{{ route('admin.add-user-level') }}" class="btn btn-primary">
                                        </i> {{ __('Tambah User Level') }}</a>
                                </div>
                            </div>
                            <div class="card-body">
                                    ...
                                    <table class="stripe row-border order-column nowrap" id="tableMaster">
                                        <thead>
                                            <tr>
                                                ...
                                                <th width="20%">{{ __('Level') }}</th>
                                                <th width="10%">{{ __('Status') }}</th>
                                                <th width="16%">{{ __('Aksi') }}</th>
                                            </tr>
                                        </thead>
                                        <tbody>          
                                            @foreach($userLevel as $index => $md)       
                                            @php 
                                            ...
                                            <tr>
                                                ...
                                                <td></td>
                                                <td>
                                                    <a href="{{ route('admin.edit-user-level', $id_mata_diklat) }}" class="btn btn-warning btn-sm">
...
@push('js')
    <script>
        function deleteData(id) {
            $("#deleteForm").attr("action", "{{ url('admin/user-level-delete') }}" + "/" + id)
        }
    </script>
@endpush