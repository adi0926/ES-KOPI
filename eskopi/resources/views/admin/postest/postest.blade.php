        ...
        <section class="section">
            <div class="section-header">
                ...
                <div class="section-header-breadcrumb">
                    ...
                    <div class="breadcrumb-item">{{ __('Postest') }}</div>
                </div>
            </div>
                    ...
                    <div class="col">
                        <div class="card">
                            <div class="card-header d-flex justify-content-between">
                                <h4>{{ __('Postest') }}</h4>
                                <div>
                                    <a href="{{ route('admin.add-postest') }}" class="btn btn-primary">
                                        {{ __('Tambah Postest') }}</a>
                                </div>
                            </div>
                            <div class="card-body">
                                <div class="table-responsive">
                                    <table class="stripe row-border order-column nowrap" id="tableMaster">
                                        ...
                                        <tbody>          
                                            @foreach($postest as $index => $md)       
                                            @php 
                                                $id_postest = Crypt::encrypt($md->id_mata_diklat);
                                            @endphp
                                            <tr>
                                                <td></td>
                                                <td>{{ ++$index }}</td>
                                                ...
                                                <td></td>
                                                <td>
                                                    <a href="{{ route('admin.edit-postest', $id_postest) }}" class="btn btn-warning btn-sm">
...
@push('js')
    <script>
        function deleteData(id) {
            $("#deleteForm").attr("action", "{{ url('admin/postest-delete') }}" + "/" + id)
        }
    </script>
@endpush